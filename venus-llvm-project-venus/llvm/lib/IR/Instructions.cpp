...
//           "Calling a function with a bad signature!");
   ...
   for(unsigned i = 0; i != Args.size(); ++i) {
     if (!(i >= FTy->getNumParams() || FTy->getParamType(i) == Args[i]->getType())) {
         llvm::errs() << "Calling a function with a bad signature! Param " << i << "\n";
         llvm::errs() << "Expected type: ";
         FTy->getParamType(i)->dump();  // 打印期望的参数类型
         llvm::errs() << "Actual type: ";
         Args[i]->getType()->dump();    // 打印实际的参数类型
         llvm::errs() << "\n";
         assert(false && "Calling a function with a bad signature!");
     }
 }
#endif
