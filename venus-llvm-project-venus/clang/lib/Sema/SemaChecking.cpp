...
void Sema::CheckVenusShuffleArgs(const CallExpr *TheCall) {

  const Expr *FirstArg = TheCall->getArg(0);
  const Expr *SecondArg = TheCall->getArg(1);
  const Expr *ThirdArg = TheCall->getArg(2);

  // 获取变量的 DeclRefExpr（变量引用）
  const DeclRefExpr *FirstDecl = dyn_cast<DeclRefExpr>(FirstArg->IgnoreParenCasts());
  const DeclRefExpr *SecondDecl = dyn_cast<DeclRefExpr>(SecondArg->IgnoreParenCasts());
  const DeclRefExpr *ThirdDecl = dyn_cast<DeclRefExpr>(ThirdArg->IgnoreParenCasts());

  // 检查变量名是否相同（即地址是否相同）
  if ((SecondDecl && FirstDecl->getDecl() == SecondDecl->getDecl()) ||
      (ThirdDecl && FirstDecl->getDecl() == ThirdDecl->getDecl())) {
      Diag(TheCall->getExprLoc(), diag::err_vshuffle_invalid_args)
          << FirstArg->getSourceRange()
          << SecondArg->getSourceRange()
          << ThirdArg->getSourceRange();
  }
}

/// SemaBuiltinVenusOverloaded - We have several calls to
/// venus builtins, which is an overloaded function.
///
/// This function goes through and does final semantic checking for these
/// builtins.
/// Specifically speaking, the return type in `BuiltinsRISCVVenus.def` is void.
/// We need this function to reload the return type according to the input
/// argument. Further IR generation is in `EmitRISCVVenusBuiltinExpr` of
/// CGBuiltin.cpp
ExprResult Sema::SemaBuiltinVenusOverloaded(ExprResult TheCallResult,
                                            unsigned BuiltinID) {
  CallExpr *TheCall = static_cast<CallExpr *>(TheCallResult.get());
  Expr *Arg;
  if (BuiltinID != RISCV::BI__Venus_barrier)
    Arg = TheCall->getArg(0);
  QualType ResultType;
  if (BuiltinID == RISCV::BI__Venus_vaddr) {
    ResultType = Context.IntTy;
  } else if (BuiltinID == RISCV::BI__Venus_barrier) {
    ResultType = Context.VoidTy;
  } else {
    ResultType = Arg->getType().getUnqualifiedType();
  }
  if (BuiltinID == RISCV::BI__Venus_shuffle_test) {
    CheckVenusShuffleArgs(TheCall);
  }
  TheCall->setType(ResultType);
  return TheCallResult;
}
