...
extern llvm::cl::opt<unsigned> VenusTileBaseAddr;
...
#define DEBUG_TYPE "venus_builtin"
inline unsigned CodeGenFunction::GetVenusOperandImm(Expr * E, StringRef Info) {
  ConstantInt *CI = dyn_cast<ConstantInt>(EmitScalarExpr(E));
  if (CI == NULL) report_fatal_error(Info + " should be an immediate.");
  const unsigned imm = CI->getZExtValue();
  LLVM_DEBUG(llvm::errs() << Info + ":\t" << imm << "\n");
  return imm;
}

Value *CodeGenFunction::EmitRISCVVenusRangeExpr(unsigned BuiltinID,
                                                const CallExpr *E) {
  CallExpr *VenusCallExpr = const_cast<CallExpr *>(E);
  const unsigned NumArgs = VenusCallExpr->getNumArgs();
  bool HasAVL = false;

  Expr *FirstArg = VenusCallExpr->getArg(0);
  const QualType FirstArgType = FirstArg->getType().getUnqualifiedType();
  const VectorType *VT = FirstArgType->getAs<VectorType>();
  const QualType FirstArgElementType = VT->getElementType();
  Expr *AVLArg;
  if (NumArgs == 2) { // An extra AVL operand
    HasAVL = true;
    AVLArg = VenusCallExpr->getArg(1);
  }

  // Get element type and rows of VRF
  unsigned const TypeSizeInChars =
      getContext().getTypeSizeInChars(FirstArgType).getQuantity();
  unsigned const ElementSizeInChars =
      getContext().getTypeSizeInChars(FirstArgElementType).getQuantity();

  if (ElementSizeInChars == 1 && BuiltinID == RISCV::BI__Venus_range)
    report_fatal_error("Wrong Element Type of the vector. Range must be i16.");

  unsigned ElementsPerRowInVenusArch = opts::VenusNrLanes
                                       * 4 /* NrBanks */ * 2 /* Bytes */;
  assert(opts::VenusNrLanes > 0 && "Invalid NrLanes");
  assert(llvm::isPowerOf2_32(ElementsPerRowInVenusArch) &&
         "NrLanes is not a power of 2");
  unsigned RowsInVenusArch =
      TypeSizeInChars >> llvm::Log2_32(ElementsPerRowInVenusArch);
  if(TypeSizeInChars & (ElementsPerRowInVenusArch - 1))
    RowsInVenusArch++;

    Value* RetVal = UndefValue::get(llvm::VectorType::get(
      Builder.getIntNTy(
          (BuiltinID == RISCV::BI__Venus_range) ? 16 :
          (ElementSizeInChars == 1) ? 8 :
          (ElementSizeInChars == 2) ? 16 :
          (ElementSizeInChars == 4) ? 32 :
          (ElementSizeInChars == 8) ? 64 :  
          0 
      ),
      TypeSizeInChars / ElementSizeInChars, false
  ));
  llvm::Type *ResultType = RetVal->getType();
  llvm::SmallVector<llvm::Type *, 4> IntrinsicTypes;
  llvm::SmallVector<llvm::Value *, 5> Operands;
  // vew is 1 (mandatory) for range
  Operands.push_back(
      (BuiltinID == RISCV::BI__Venus_range) ? (Builder.getInt32(1)) :
      (Builder.getInt32((ElementSizeInChars == 2)))
      );
  Operands.push_back(Builder.getInt32(0)); // mask_read is 0 (mandatory)

  unsigned IID_RangeType;
  switch (BuiltinID) {
    case RISCV::BI__Venus_claim: {
      Operands.push_back(Builder.getInt32(opts::ClaimInputCounter)); // sub-intrinsic ID
      opts::ClaimInputCounter += opts::VenusNrRows;
      IID_RangeType = Intrinsic::venus_claim_misc; break;
    }
    case RISCV::BI__Venus_range: {
      Operands.push_back(Builder.getInt32(0)); // sub-intrinsic ID
      IID_RangeType = Intrinsic::venus_range_misc; break;
    }
  }

  if (HasAVL) {
    llvm::Value *CastValue = Builder.CreateZExt(EmitScalarExpr(AVLArg),
                                                Builder.getIntNTy(32));
    Operands.push_back(CastValue);
  }
  else        Operands.push_back(Builder.getInt32(TypeSizeInChars/ElementSizeInChars));

  // Number of Rows
  Operands.push_back(Builder.getInt32(RowsInVenusArch));

  const unsigned IID = IID_RangeType;
  IntrinsicTypes = {ResultType, Operands[0]->getType(), Operands[2]->getType()};
  Function *F = CGM.getIntrinsic(IID, IntrinsicTypes);

//  // The number of rows of intrinsic to be chosen is determined by return value.
//  QualType RetType = VenusCallExpr->getType().getUnqualifiedType();
//  QualType VecType = getContext().getBaseElementType(RetType);
//  LLVM_DEBUG(llvm::errs() << VecType.getAsString() << "\n");

  return Builder.CreateCall(F, Operands, "");
}

Value *CodeGenFunction::EmitRISCVVenusMnotExpr(unsigned BuiltinID,
                                               const CallExpr *E) {
  CallExpr *VenusCallExpr = const_cast<CallExpr *>(E);
  const unsigned NumArgs = VenusCallExpr->getNumArgs();
  bool HasAVL = false;
  ...
  unsigned const ElementSizeInChars =
      getContext().getTypeSizeInChars(FirstArgElementType).getQuantity();
  bool isElementShort = false;
  bool isElementInt = false;
  bool isElementDouble = false;
  if (ElementSizeInChars == 1)      isElementShort = false;
  else if (ElementSizeInChars == 2) isElementShort = true;
  else if (ElementSizeInChars == 4) isElementInt = true;
  else if (ElementSizeInChars == 8) isElementDouble = true;
  else                              assert(0 && "Wrong Element Type of the vector.");
  ...
  llvm::SmallVector<llvm::Type *, 4> IntrinsicTypes;
  llvm::SmallVector<llvm::Value *, 5> Operands;
  Operands.push_back(Builder.getInt32(isElementShort)); // vew
  Operands.push_back(Builder.getInt32(0)); // mask_read is 0 (mandatory)
  if (HasAVL) {
    llvm::Value *CastValue = Builder.CreateZExt(EmitScalarExpr(AVLArg),
                                                Builder.getIntNTy(32));
  ...
  IntrinsicTypes = {Operands[0]->getType(), Operands[2]->getType()};
  Function *F = CGM.getIntrinsic(Intrinsic::venus_mnot_misc, IntrinsicTypes);

  return Builder.CreateCall(F, Operands, "");
}

Value *CodeGenFunction::EmitRISCVVenusShuffleExpr(unsigned BuiltinID,
                                                  const CallExpr *E) {
  CallExpr *VenusCallExpr = const_cast<CallExpr *>(E);
  const unsigned NumArgs = VenusCallExpr->getNumArgs();
  bool HasAVL = false;

  Expr *FirstArg = VenusCallExpr->getArg(0);   // Return Vec
  Expr *SecondArg = VenusCallExpr->getArg(1);  // Index Vec
  Expr *ThirdArg = VenusCallExpr->getArg(2);   // Target Vec
  const QualType SecondArgType = SecondArg->getType().getUnqualifiedType();
  const QualType ThirdArgType = ThirdArg->getType().getUnqualifiedType();
  const VectorType* VT = ThirdArgType->getAs<VectorType>();
  QualType SecondArgElementType;
  QualType ThirdArgElementType = VT->getElementType();
  Expr *FourthArg = VenusCallExpr->getArg(3);       // Gather/Scatter Indicator
  if (SecondArgType->isVectorType()) {
    VT = SecondArgType->getAs<VectorType>();
    SecondArgElementType = VT->getElementType();
    if (getContext().getTypeSizeInChars(
                        SecondArg->getType().getUnqualifiedType()
                            ->getAs<VectorType>()->getElementType()).getQuantity() == 1)
      report_fatal_error("Element Type of the Index Vector must be short.");
  }

  Expr *AVLArg;
  if (NumArgs == 5) { // An extra AVL operand
    HasAVL = true;
    AVLArg = VenusCallExpr->getArg(4);
  }

  // Get element type and rows of VRF
  unsigned const TypeSizeInChars =
      getContext().getTypeSizeInChars(ThirdArgType).getQuantity();
  unsigned const ElementSizeInChars =
      getContext().getTypeSizeInChars(ThirdArgElementType).getQuantity();
  bool isElementShort = false;
  bool isElementInt = false;
  bool isElementDouble = false;
  if (ElementSizeInChars == 1)      isElementShort = false;
  ...
  llvm::SmallVector<llvm::Type *, 4> IntrinsicTypes;
  llvm::SmallVector<llvm::Value *, 5> Operands;
  llvm::Type *ResultType = ConvertType(FirstArg->getType().getUnqualifiedType());
  if (!SecondArgType->isVectorType()) {
    llvm::Value *CastValue = Builder.CreateZExt(EmitScalarExpr(SecondArg),
                                                Builder.getIntNTy(32));
    Operands.push_back(CastValue);
  } else {
    Operands.push_back(EmitScalarExpr(SecondArg));
  }
  Operands.push_back(EmitScalarExpr(ThirdArg));
  Operands.push_back(EmitScalarExpr(FirstArg));
  Operands.push_back(Builder.getInt32(isElementShort)); // vew
  Operands.push_back(EmitScalarExpr(FourthArg)); // mask_read is changed to sg
  if (HasAVL) {
    llvm::Value *CastValue = Builder.CreateZExt(EmitScalarExpr(AVLArg),
                                                Builder.getIntNTy(32));
    Operands.push_back(CastValue);
  ...
  IntrinsicTypes = {ResultType, Operands[0]->getType(), Operands[1]->getType(),
                                Operands[3]->getType(), Operands[5]->getType()};

  const unsigned IID = ThirdArgType->isVectorType() ? Intrinsic::venus_shuffle_ivv :
                                                    Intrinsic::venus_shuffle_ivx;
  Function *F = CGM.getIntrinsic(IID, IntrinsicTypes);

  return Builder.CreateCall(F, Operands, "");
}

Value *CodeGenFunction::EmitRISCVVenusVaddrExpr(unsigned BuiltinID,
                                                const CallExpr *E,
                                                unsigned ArgOffset) {
  CallExpr *VenusCallExpr = const_cast<CallExpr *>(E);

  Expr *FirstArg = VenusCallExpr->getArg(ArgOffset);  // vector to be accessed
  const QualType FirstArgType = FirstArg->getType().getUnqualifiedType();
  ...
  llvm::SmallVector<llvm::Type *, 4> IntrinsicTypes;
  llvm::SmallVector<llvm::Value *, 5> Operands;
  llvm::Value *VecValue = EmitScalarExpr(FirstArg);
  IntrinsicTypes = {VecValue->getType()};
  Operands = {VecValue};
  const unsigned IID = Intrinsic::venus_vaddr_misc;
  Function *F = CGM.getIntrinsic(IID, IntrinsicTypes);

  return Builder.CreateCall(F, Operands, "");
}

Value *CodeGenFunction::EmitRISCVVenusRetExpr(unsigned BuiltinID,
                                              const CallExpr *E) {
  CallExpr *VenusCallExpr = const_cast<CallExpr *>(E);

  const unsigned CRBaseAddr = opts::VenusCRBaseAddr;
  const unsigned RstOffset = 0x0;
  const unsigned TskCRCOffset = 0x28;
  const unsigned NumRetOffset = 0x2C;
  const unsigned RetAddr0Offset = 0x30;
  const unsigned RetLen0Offset = 0x34;

  unsigned NumOperands;
  if (VenusCallExpr->getArg(0)->getType()->isVoidPointerType())
    NumOperands = 0;
  else
    NumOperands = VenusCallExpr->getNumArgs();

  if (NumOperands & 0x1) {
    report_fatal_error("NumArgs of venus_return must be an even number!\n");
  }
  const unsigned NumRetSets = NumOperands >> 1;

  Builder.CreateCall(CGM.getIntrinsic(Intrinsic::venus_barrier, {}), {}, "");

  int hashvalue = 0;
  // Check VenusTaskName, no need to calculate hash if it is empty
  if (!opts::VenusTaskName.empty()) {
    for (char ch : opts::VenusTaskName) {
      srand((unsigned int)ch);
      hashvalue += rand();
    }
    // Write the hashvalue to a file
    std::ofstream hashfile(opts::VenusTaskName + ".hash");
    std::ofstream debug_output_file(opts::VenusTaskName + "_output_num.txt");

    int real_output_NumOperands = NumOperands / 2;

    LLVM_DEBUG(dbgs() << "Function: " << opts::VenusTaskName << "\n");
    LLVM_DEBUG(dbgs() << "NumOperands: " << NumOperands << "\n");
    LLVM_DEBUG(dbgs() << "real_output_NumOperands: " << real_output_NumOperands << "\n");

    if (debug_output_file.is_open()) {
      debug_output_file << real_output_NumOperands << "\n";
      debug_output_file.close();
    } else {
      report_fatal_error("Cannot write hash value to a file!");
    }

    if (hashfile.is_open()) {
      hashfile << std::hex << hashvalue << "\n";
      hashfile.close();
    } else {
      report_fatal_error("Cannot write hash value to a file!");
    }
    // Write the hashvalue to the CSR
    llvm::Value *TskCRCRegValue = Builder.getInt32(CRBaseAddr + TskCRCOffset);
    llvm::Value *TskCRCPtrValue = Builder.CreateIntToPtr(TskCRCRegValue,
                                                         Builder.getPtrTy());
    Builder.CreateAlignedStore(Builder.getInt32(hashvalue),
                               TskCRCPtrValue, CharUnits::fromQuantity(4), true);
  }

  llvm::Value *NumRetRegValue = Builder.getInt32(CRBaseAddr + NumRetOffset);
  llvm::Value *NumRetPtrValue = Builder.CreateIntToPtr(NumRetRegValue,
                                                     Builder.getPtrTy());
  Builder.CreateAlignedStore(Builder.getInt32(NumRetSets),
                             NumRetPtrValue, CharUnits::fromQuantity(4), true);

  for (unsigned RetSetsCount = 0, RemainOperands = NumOperands - 2;
       RetSetsCount < NumRetSets; RetSetsCount++, RemainOperands-=2) {
    Expr *RetArg = VenusCallExpr->getArg(RetSetsCount << 1);
    const QualType RetArgType = RetArg->getType().getUnqualifiedType();
    llvm::Value *RetArgValue;
    llvm::Value *AddrRegValue = Builder.getInt32(CRBaseAddr
                                                 + RetAddr0Offset
                                                 + (RetSetsCount << 3));
    llvm::Value *LenRegValue = Builder.getInt32(CRBaseAddr
                                                 + RetLen0Offset
                                                 + (RetSetsCount << 3));
    llvm::Value *AddrPtrValue = Builder.CreateIntToPtr(AddrRegValue,
                                                       Builder.getPtrTy());
    llvm::Value *LenPtrValue = Builder.CreateIntToPtr(LenRegValue,
                                                      Builder.getPtrTy());
    Expr *LenArg = VenusCallExpr->getArg((RetSetsCount << 1) + 1);
    if (RetArgType->isVectorType()) {
      RetArgValue = EmitRISCVVenusVaddrExpr(BuiltinID, VenusCallExpr,
                                            RetSetsCount << 1);
    } else {
      RetArgValue = Builder.CreatePtrToInt(EmitScalarExpr(RetArg),
                                           Builder.getIntNTy(32));
    }


    // Only want relative address
    RetArgValue = Builder.CreateSub(RetArgValue,
                                    Builder.getInt32(opts::VenusTileBaseAddr));
    Builder.CreateAlignedStore(RetArgValue, AddrPtrValue,
                               CharUnits::fromQuantity(4), true);
    Builder.CreateAlignedStore(EmitScalarExpr(LenArg), LenPtrValue,
                               CharUnits::fromQuantity(4), true);
  }
  // Self reset the core through configuring the CSR
  llvm::Value *RstRegValue = Builder.getInt32(CRBaseAddr + RstOffset);
  llvm::Value *RstPtrValue = Builder.CreateIntToPtr(RstRegValue,
                                                    Builder.getPtrTy());
//  Value *RetValue = Builder.CreateAlignedStore(
//    Builder.getInt32(llvm::Log2_32(opts::VenusNrLanes) << 8),
//    RstPtrValue, CharUnits::fromQuantity(4), true);
  Value *RetValue = Builder.CreateAlignedStore(
      Builder.getInt32(0),
      RstPtrValue, CharUnits::fromQuantity(4), true);
...
Value *CodeGenFunction::EmitRISCVVenusComplexExpr(unsigned BuiltinID,
                                                  const CallExpr *E) {
  ...
  Expr *FirstArg = VenusCallExpr->getArg(0);
  Expr *SecondArg = VenusCallExpr->getArg(1);
  Expr *ThirdArg = VenusCallExpr->getArg(2);
  Expr *FourthArg = VenusCallExpr->getArg(3);
  Expr *FifthArg = VenusCallExpr->getArg(4);
  Expr *SixthArg = VenusCallExpr->getArg(5);
  const QualType FirstArgType =
      ThirdArg->getType().getUnqualifiedType();  // ThirdArg is the real input.
  const VectorType *VT = FirstArgType->getAs<VectorType>();
  const QualType FirstArgElementType = VT->getElementType();

  Expr *AVLArg;
  if (NumArgs == 8) { // An extra AVL operand
    HasAVL = true;
    AVLArg = VenusCallExpr->getArg(7);
  }
  ...
  llvm::SmallVector<llvm::Type *, 8> IntrinsicTypes;
  llvm::SmallVector<llvm::Value *, 8> Operands;

  llvm::Type *ResultType = ConvertType(FirstArg->getType()->getPointeeType());
 
  Operands.push_back(EmitScalarExpr(ThirdArg));
  Operands.push_back(EmitScalarExpr(FourthArg));
  Operands.push_back(EmitScalarExpr(FifthArg));
  Operands.push_back(EmitScalarExpr(SixthArg));
  Operands.push_back(Builder.getInt32(isElementShort)); // vew
  Operands.push_back(Builder.getInt32(
      GetVenusOperandImm(VenusCallExpr->getArg(6), "Mask Read"))); // mask_read is 0 (mandatory)
  if (HasAVL) {
    llvm::Value *CastValue = Builder.CreateZExt(EmitScalarExpr(AVLArg),
                                                Builder.getIntNTy(32));
    Operands.push_back(CastValue);
  ...
  IntrinsicTypes = {ResultType, Operands[4]->getType()};

  const unsigned IID = Intrinsic::venus_cmxmul_ivv;
  Function *F = CGM.getIntrinsic(IID, IntrinsicTypes);
  llvm::Value *CallValue = Builder.CreateCall(F, Operands, "");
  clang::CharUnits Align =
            CGM.getNaturalPointeeTypeAlignment(E->getArg(0)->getType());
 // Extract individual values from the struct
  llvm::Value *Result;

  llvm::Value *Val = Builder.CreateExtractValue(CallValue, {0});
  Result = Builder.CreateStore(Val, Address(EmitScalarExpr(FirstArg), Val->getType(), Align));
  Val = Builder.CreateExtractValue(CallValue, {1});
  Result = Builder.CreateStore(Val, Address(EmitScalarExpr(SecondArg), Val->getType(), Align));  

  return Result;
}

Value *CodeGenFunction::EmitRISCVVenusShuffleTestExpr(unsigned BuiltinID,
                                                      const CallExpr *E) {
  CallExpr *VenusCallExpr = const_cast<CallExpr *>(E);
  const unsigned NumArgs = VenusCallExpr->getNumArgs();
  bool HasAVL = false;
  ...
  * 4 /* NrBanks */ * 2 /* Bytes */;
  ...
  IntrinsicTypes = {ResultType, Operands[0]->getType(), Operands[1]->getType(), Operands[3]->getType(), Operands[5]->getType()};

  const unsigned IID = Intrinsic::venus_shuffle_test;
  Function *F = CGM.getIntrinsic(IID, IntrinsicTypes);

  return Builder.CreateCall(F, Operands, "");
}

Value *CodeGenFunction::EmitRISCVVenusBuiltinExpr(unsigned BuiltinID,
                                                  const CallExpr *E) {
  CallExpr *VenusCallExpr = const_cast<CallExpr *>(E);

  switch(BuiltinID) {
    case RISCV::BI__Venus_mnot:  return EmitRISCVVenusMnotExpr(BuiltinID, E);
    case RISCV::BI__Venus_claim:
    case RISCV::BI__Venus_range: return EmitRISCVVenusRangeExpr(BuiltinID, E);
    case RISCV::BI__Venus_shuffle: return EmitRISCVVenusShuffleExpr(BuiltinID, E);
    case RISCV::BI__Venus_shuffle_test: return EmitRISCVVenusShuffleTestExpr(BuiltinID, E);
    case RISCV::BI__Venus_vaddr: return EmitRISCVVenusVaddrExpr(BuiltinID, E, 0);
    case RISCV::BI__Venus_return: return EmitRISCVVenusRetExpr(BuiltinID, E);
    case RISCV::BI__Venus_cmxmul: return EmitRISCVVenusComplexExpr(BuiltinID, E);
    case RISCV::BI__Venus_barrier:
      return Builder.CreateCall(CGM.getIntrinsic(Intrinsic::venus_barrier, {}), {}, "");
    case RISCV::BI__Venus_setcsr:
      return Builder.CreateCall(
          CGM.getIntrinsic(
              Intrinsic::venus_setcsr, {}),
          {EmitScalarExpr(VenusCallExpr->getArg(0)), EmitScalarExpr(VenusCallExpr->getArg(1))}, "");
  }
  ...
  Expr *FourthArg;
  Expr *AVLArg;
  QualType FirstArgType = FirstArg->getType().getUnqualifiedType();
  QualType SecondArgType = SecondArg->getType().getUnqualifiedType();
  QualType FirstArgElementType, SecondArgElementType;
  if (FirstArgType->isVectorType()) {
    const VectorType *VT = FirstArgType->getAs<VectorType>();
    FirstArgElementType = VT->getElementType();
  } else {
  ...
  unsigned vmr = 0; // vmask read
  unsigned vmw = 0; // vmask write
  unsigned vewOperandIndex = 2;
  unsigned avlOperandIndex = 4;
  llvm::Type *ResultType = ConvertType(FirstArgType);
  llvm::SmallVector<llvm::Type *, 4> IntrinsicTypes;
  llvm::SmallVector<llvm::Value *, 5> Operands;
  Operands.push_back(EmitScalarExpr(FirstArg));   // source operand 1
  if (!SecondArgType->isVectorType()) {
    llvm::Value *CastValue = Builder.CreateZExt(EmitScalarExpr(SecondArg),
                                                Builder.getIntNTy(32));
    Operands.push_back(CastValue);
  } else {
    // `BI__Venus_brdcst` is an IVX intrin intrinsicsic
    if (BuiltinID == RISCV::BI__Venus_brdcst)
      report_fatal_error("The second arg of venus_brdcst can only be a scalar");
    Operands.push_back(EmitScalarExpr(SecondArg));  // source operand 2
  }
  // We avoided to declare new types of immediate numbers at the back end.
  // Even though a 32-bit imm is declared here, we still truncated to bits
  // specified in the `venus_encoding` spreadsheet.
  // Check `llvm/IR/RISCVInstrFormatsVenus.td`
  if (BuiltinID >= RISCV::BI__Venus_muladd &&
      BuiltinID <= RISCV::BI__Venus_submul) {
    vewOperandIndex = 3;
    avlOperandIndex = 5;
    Operands.push_back(EmitScalarExpr(ThirdArg));   // source operand 3
    Operands.push_back(Builder.getInt32(isElementShort)); // vew
    FourthArg = VenusCallExpr->getArg(3);
    vmr = GetVenusOperandImm(FourthArg, "Mask Read");
    if (NumArgs == 5) { // An extra AVL operand
      HasAVL = true;
      AVLArg = VenusCallExpr->getArg(4);
    }
    ...
    Operands.push_back(Builder.getInt32(isElementShort)); // vew
    vmr = GetVenusOperandImm(ThirdArg, "Mask Read");
    // An extra AVL operand for non-MVV/MVXs
    if (NumArgs == 4 && !(BuiltinID >= RISCV::BI__Venus_seq &&
                          BuiltinID <= RISCV::BI__Venus_sgt)) {
      HasAVL = true;
      AVLArg = VenusCallExpr->getArg(3);
    }
  }
  Operands.push_back(Builder.getInt32(vmr));

  // vmask_write aka MVV/MVX
  if (BuiltinID >= RISCV::BI__Venus_seq &&
      BuiltinID <= RISCV::BI__Venus_sgt) {
    FourthArg = VenusCallExpr->getArg(3);
    vmw = GetVenusOperandImm(FourthArg, "Mask Write");
    if (NumArgs == 5) { // An extra AVL operand
      HasAVL = true;
      AVLArg = VenusCallExpr->getArg(4);
    }
  ...
  // Select Intrinsic ID
  unsigned IID;
  unsigned IIDOffset, IIDOffsetVX, IIDOffsetMW;
  // Vector-scalar intrinsics offset. Check `IntrinsicEnum.inc`
  IIDOffsetVX = SecondArgType->isVectorType() ? 0 : 1;
  // Mask write intrinsics offset. Check `IntrinsicEnum.inc`
  IIDOffsetMW = vmw ? (1 << 1) : 0;
  IIDOffset = IIDOffsetVX + IIDOffsetMW;
  switch(BuiltinID) {
  default: llvm_unreachable("Unknown venus builtin!");
#define VENUS_BUILTIN(ID, TYPE, ATTRS, FEATURE, INTRINSIC, MASK_WRITE)          \
    case RISCV::BI##ID: {                                                       \
      IID = Intrinsic::INTRINSIC + IIDOffset;                                   \
      break;                                                                    \
    }
#include "clang/Basic/BuiltinsRISCVVenus.def"
  }

  IntrinsicTypes = {ResultType, Operands[vewOperandIndex]->getType(),
                                Operands[avlOperandIndex]->getType()};
  Function *F = CGM.getIntrinsic(IID, IntrinsicTypes);

  QualType VecType = getContext().getBaseElementType(FirstArgType);
  LLVM_DEBUG(llvm::errs() << "NumArgs:\t" << NumArgs << "\n");
  LLVM_DEBUG(llvm::errs() << VecType.getAsString() << "\n");
  LLVM_DEBUG(llvm::errs() << "ElementSizeInChars:\t" << ElementSizeInChars << "\n");
  LLVM_DEBUG(llvm::errs() << "TypeSizeInChars:\t" << TypeSizeInChars << "\n");
  LLVM_DEBUG(llvm::errs() << "ElementsPerRowInVenusArch:\t" << ElementsPerRowInVenusArch << "\n");
  LLVM_DEBUG(llvm::errs() << "RowsInVenusArch:\t" << RowsInVenusArch << "\n\n");
  return Builder.CreateCall(F, Operands, "");
}
#undef DEBUG_TYPE
...
#define VENUS_BUILTIN(ID, TYPE, ATTRS, FEATURE, INTRINSIC, MASK_WRITE) \
  case RISCV::BI##ID:
#include "clang/Basic/BuiltinsRISCVVenus.def"
    return EmitRISCVVenusBuiltinExpr(BuiltinID, E);
